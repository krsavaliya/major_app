// ignore_for_file: unused_import, prefer_const_constructors, unnecessary_new, curly_braces_in_flow_control_structures
...
class TeacherBasicSecPage extends StatefulWidget {
  ...
  //a te code lidho jyathi ema pass krela hse,, ema error hati aa solve karvama 'requred' hatu
  TeacherBasicSecPage(
      {required Key key,
      required this.auth,//a paarameter compulsory pass karva pde to j a class call thay
    ...
    for (int i = 0; i < idList.length; i++) {
      if (idList[i].data()['role'] == 'student') {
        var y = await FirebaseFirestore.instance
            ...
            .doc(queryMonth)
            .get();
        double subCount = 0; // Initialize as double
        for (int j = 1; j <= 30; j++) {
          if (j < 10)
            ...
            j1 = j.toString();
          try {
            subCount += (y.data()?[j1]['count'] as double);// Cast the right-hand type to double
          } catch (e) {
            continue;
          }
        }
